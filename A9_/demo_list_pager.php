...
$myPager = new Pager(2,'20',$prev,$next,'');
...
{#records exist - process
	...
	if($myPager->showTotal()==1){$itemz = "kitten";}else{$itemz = "kittens";}  //deal with plural
    echo '<p align="center">We have ' . $myPager->showTotal() . ' ' . $itemz . '!</p>';
	while($row = mysqli_fetch_assoc($result))
	{# process each row
         echo '<p align="center">
            <a href="db_view.php?id=' . (int)$row['KittensID'] . '"><img src="img/Kittens' . (int)$row['KittensID'] . '.jpg" style="max-width:50px" /> ' . $row['FirstName'] . '</a>
            </p>';
	}
    ...
    echo "<p align=center>What! No Kittens?  There must be a mistake!!</p>";	
}
